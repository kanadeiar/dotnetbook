...
Событие объявляется ключевым словом event, назначается область дейтвия, тип делегата, указывающий на прототип вызываемого метода и имя. Получатели уведомления о событии должны предоставлять метод обратного вызова, прототип которого соответствует делегату EventHandler<T>. 
